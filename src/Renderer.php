 ...
 *
 * Either you can choose between some image formats (png, gif, jpg) or XML. The
 * XML format is the direct syntax tree representation generated by the parser.
 *
 * @package ebnf
 */
class Renderer {
    ...
    private function renderNode(DOMElement $node, $leftToRight) {
        if ($node->nodeName === Type::IDENTIFIER || $node->nodeName === Type::TERMINAL) {
            return $this->renderIdentifierOrTerminal($node);
        } else if ($node->nodeName === Type::OPTION || $node->nodeName === Type::LOOP) {
            return $this->renderOptionOrLoopNode($node, $leftToRight);
        } else if ($node->nodeName === Type::SEQUENCE) {
            return $this->renderSequenceNode($node, $leftToRight);
        } else if ($node->nodeName === Type::CHOICE) {
            return $this->renderChoiceNode($node, $leftToRight);
        } else if ($node->nodeName === Type::SYNTAX) {
    ...
    private function renderIdentifierOrTerminal($node) {
        ...
        if ($node->nodeName !== Type::TERMINAL) {
